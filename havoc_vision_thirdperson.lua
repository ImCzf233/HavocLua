...
--region dependency: havoc_vector_2_10_0
--region ffi
...
function vector_old_c:closest_ray_point(source, destination)
	local to = self - source
	local direction = destination - source
	...
	return source + direction * ray_along
end
...
--- @param destination vector_old_c
--- @return boolean
function vector_old_c:intersects_smoke(destination)
	return line_goes_through_smoke(self.x, self.y, self.z, destination.x, destination.y, destination.z, 1)
end
...
--- @param eid number
--- @return vector_old_c
function vector_old_c.eye_position(eid)
...
--region setup
local view_pos = vector(client.eye_position())
--endregion

--region crosshairs
local crosshairs = {
	["Standard"] = renderer.load_png(
		hex_to_bin(""), 32, 32
	),
	["Standard (Shadow)"] = renderer.load_png(
		hex_to_bin(""), 32, 32
	),
	["Tilted"] = renderer.load_png(
		hex_to_bin(""), 32, 32
	),
	["Tilted (Shadow)"] = renderer.load_png(
		hex_to_bin(""), 32, 32
	),
}

local crosshair_tex = crosshairs["Standard"]
local crosshair_pos = vector(client.screen_size()) / 2
--endregion
...
menu:label("--------------------------------------------------")
menu:label("Smooth Cam - v1.2.0")

local enable_script = menu:checkbox("Enable Smooth Cam")

local cam_dist = menu:slider("Camera distance", 32, 200, {
	default = 100
})

local cam_hor =  menu:slider("Camera horizontal offset", -30, 30, {
	default = 0
})

local cam_swap_hor_state = false
local cam_swap_hor = menu:hotkey("Camera swap left/right", false)

local cam_ver =  menu:slider("Camera vertical offset", -50, 50, {
	default = 0
})

local cam_slack =  menu:slider("Camera slack", 1, 100, {
	default = 40
})

local thirdperson_while_scoped = menu:checkbox("Allow thirdperson when scoped")

local crosshair = menu:checkbox("Accurate crosshair")


local crosshair_color = menu:color_picker("Crosshair color", 255, 255, 0, 255)
local crosshair_color_enemy = menu:color_picker("Crosshair on-enemy color", 255, 255, 0, 255)

local crosshair_size = menu:slider("Crosshair size", 8, 32, {
	default = 20,
	unit = "px"
})

local crosshair_type = menu:combobox("Crosshair type", {
	"Standard", "Standard (Shadow)", "Tilted", "Tilted (Shadow)"
})

crosshair_type:add_callback(function()
	crosshair_tex = crosshairs[crosshair_type()]
end)

crosshair:add_children({
	crosshair_color,
...
enable_script:add_children({
	cam_dist, cam_hor, cam_swap_hor, cam_ver, cam_slack, crosshair, thirdperson_while_scoped
})

local thirdperson, thirdperson_hotkey = ui.reference("visuals", "effects", "force third person (alive)")
...
client.set_event_callback("paint", function()
	...
	if (cam_swap_hor() ~= cam_swap_hor_state) then
		cam_swap_hor_state = cam_swap_hor()
	...
	if (ui.get(fakeduck) == true) then
		local origin_z = select(3, entity.get_origin(entity.get_local_player()))

		eye_pos.z = origin_z + 46
	end

	local cam_angles = angle(client.camera_angles())

	local offset = cam_angles:to_backward_vector() * cam_dist() +
		cam_angles:to_right_vector() * cam_hor() +
		cam_angles:to_up_vector() * cam_ver()

	local cam_trace = eye_pos:trace_line_skip(eye_pos + offset, function(eid)
		return eid > 0 and eid <= 64
	end, 64)

	view_pos = view_pos + ((eye_pos + offset * (cam_trace * 0.8)) - view_pos) * ((cam_slack()) * 0.001)

	if (crosshair() == false) then
		return
	end

	if (thirdperson_while_scoped() == false and entity.get_prop(entity.get_local_player(), "m_bIsScoped") == 1) then
		return
	end

	local aim_punch = angle(entity.get_prop(entity.get_local_player(), "m_aimPunchAngle")) * 2
	local view_offset = ((cam_angles + aim_punch):to_forward_vector()) * 32768
	local crosshair_trace, crosshair_hit = eye_pos:trace_line_to(eye_pos + view_offset, entity.get_local_player())
	local crosshair_s
	local crosshair_pos_t = eye_pos + view_offset * crosshair_trace

	if (eye_pos:intersects_smoke(eye_pos + view_offset * crosshair_trace) == true) then
		crosshair_hit = nil
	end

	if (ui.get(thirdperson) == false or ui.get(thirdperson_hotkey) == false) then
		crosshair_s = vector(client.screen_size()) / 2
	else
		crosshair_s = crosshair_pos_t:to_screen()
	end

	local r, g, b, a

	if (entity.is_enemy(crosshair_hit) == true) then
		r, g, b, a = crosshair_color_enemy()
	else
		r, g, b, a = crosshair_color()
	end

	if (crosshair_s ~= nil) then
		crosshair_pos = crosshair_pos + (crosshair_s - crosshair_pos) * 0.08

		local size = crosshair_size()

		renderer.texture(
			crosshair_tex,
			crosshair_pos.x - (size / 2), crosshair_pos.y - (size / 2),
			size, size, r, g, b, a, "f"
		)
	end
end)
...
client.set_event_callback("override_view", function(cmd)
	...
	if (thirdperson_while_scoped() == false and entity.get_prop(entity.get_local_player(), "m_bIsScoped") == 1) then
		ui.set(thirdperson, false)

		return
	else
		ui.set(thirdperson, true)
	end
