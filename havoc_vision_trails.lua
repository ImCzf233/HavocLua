...
--region globals
local illusion = require("havoc_illusion")("trails")
local last_spawn_vector = illusion.vector()
--endregion
...
menu:label("--------------------------------------------------")
menu:label("Player Trails - v1.0.1-beta")

local enable_script = menu:checkbox("Enable Player Trails")

local trail_shader = illusion.shader.rgb(255, 255, 255, 5)
local trail_color = menu:color_picker("Trail color", 255, 255, 255, 5)

trail_color:add_callback(function()
	local r, g, b, a = trail_color()

	trail_shader(r, g, b, a)
end)

client.delay_call(0.015625, function()
	local r, g, b, a = trail_color()

	trail_shader(r, g, b, a)
end)

local trail_rainbow = menu:checkbox("Trail rainbow mode")
local trail_rainbow_speed_value = 0
local trail_rainbow_speed = menu:slider("Trail rainbow speed", 1, 10, {
	unit = "x"
})

trail_rainbow_speed:add_callback(function()
	trail_rainbow_speed_value = trail_rainbow_speed() * 0.05
end)

local trail_radius = menu:slider("Trail radius", 1, 30, {
	default = 20,
	unit = "x"
})

local trail_decay_speed_value = 0
local trail_decay_speed = menu:slider("Trail decay speed", 1, 50, {
	default = 5,
	scale = 0.1,
	unit = "x"
})

trail_decay_speed:add_callback(function()
	trail_decay_speed_value = trail_decay_speed() * 0.1
end)
...
client.set_event_callback("paint", function()
	...
	if (trail_rainbow() == true) then
		trail_shader:shift_hue(trail_rainbow_speed_value * illusion.simulation.delta)
	end

	if (illusion.shared.is_dead == true or illusion.shared.player_speed < 30) then
		return
	end

	if (last_spawn_vector:distance(illusion.shared.player_origin) < 0.5) then
		return
	end

	last_spawn_vector = illusion.shared.player_origin:clone()

	local trail = illusion:create()

	trail.origin = illusion.shared.player_origin:clone()
	trail.shader = trail_shader:clone()
	trail.type = "circle"
	trail.radius = trail_radius()
	trail.on_frame = function()
		trail:kill_invisible()

		trail.shader:fade_out(trail_decay_speed_value * illusion.simulation.delta)
	end
end)
