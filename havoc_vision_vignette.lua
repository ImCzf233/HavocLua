...
menu:label("--------------------------------------------------")
menu:label("Vignette - v1.0.0")

local enable_script = menu:checkbox("Enable Vignette")

local vignette_color = menu:color_picker("Vignette color", 0, 0, 0, 100)
local vignette_size = menu:slider("Vignette size", 1, 100, {
	default = 25,
	unit = "%"
})
...
client.set_event_callback("paint", function()
	if (enable_script() == false) then
		return
	end

	local r, g, b, a = vignette_color()
	local screen_x, screen_y = client.screen_size()
	local size = 600 * (vignette_size() * 0.01)
