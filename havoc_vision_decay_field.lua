...
--- Loads the values for menu items currently from to the database.
--- @return void
...
local illusion = require("havoc_illusion")("decay field")

--region menu
local menu = menu_manager_c.new("config", "presets")

menu:label("--------------------------------------------------")
menu:label("Decay Field - v1.0.4")

local enable_script = menu:checkbox("Enable Decay Field")

local field_shader = illusion.shader.rgb(255, 255, 255, 255)
local field_color = menu:color_picker("Field color", 255, 255, 255, 255)

field_color:add_callback(function()
	local r, g, b, a = field_color()

	field_shader:set_r(r)
	field_shader:set_g(g)
	field_shader:set_b(b)
	field_shader:set_a(a)
end)

local spawn_rate_value = 0
local spawn_rate = menu:slider("Field spawn rate", 10, 100, {
	default = 50,
	unit = "%"
})

spawn_rate:add_callback(function()
	spawn_rate_value = spawn_rate()
end)

local field_speed_value = 0
local field_speed = menu:slider("Field speed", 1, 20, {
	default = 10,
	unit = "x",
	scale = 0.1
})

field_speed:add_callback(function()
	field_speed_value = field_speed() * 0.1
end)

local field_radius_value = 0
local field_radius = menu:slider("Field radius", 1, 50, {
	default = 12,
	unit = "x",
	scale = 0.1
})

field_radius:add_callback(function()
	field_radius_value = field_radius() * 0.1
end)
...
client.set_event_callback("paint", function()
	...
	if (client.random_int(0, 100) > spawn_rate_value) then
		return
	end

	if (illusion.shared.player_speed < client.random_int(0, 450)) then
		return
	end

	local spawn_vector = illusion.shared.player_origin:clone_offset(
		client.random_int(-16, 16),
		client.random_int(-16, 16),
		client.random_int(0, 64)
	)

	local decay = illusion:create()

	decay.origin = spawn_vector
	decay.shader = field_shader:clone()
	decay.type = "circle"
	decay.radius = field_radius_value
	decay.timer = illusion.timer_standard(true)
	decay.fade_time = 0.5
	decay.target_vector = illusion.vector(0, 0, 0.33)
	decay.on_frame = function()
		decay:kill_invisible()

		decay.shader:fade_out(1 * illusion.simulation.delta)
		decay.shader:desaturate(0.0025 * illusion.simulation.delta)

		decay.timer:event(0.33, function()
			decay.target_vector(
				client.random_float(-0.33, 0.33),
				client.random_float(-0.33, 0.33),
				client.random_float(0.1, 0.25)
			)

			decay.timer:restart()
		end)

		decay.origin = decay.origin + decay.target_vector * field_speed_value * illusion.simulation.delta
	end
end)
