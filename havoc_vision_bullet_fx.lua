...
menu:label("--------------------------------------------------")
menu:label("Bullet FX - v1.0.2-beta")

local enable_script = menu:checkbox("Enable Bullet FX")(true)

local player_fx_value = false
local player_fx = menu:checkbox("Player FX")

player_fx:add_callback(function()
	player_fx_value = player_fx()
end)

local player_color = menu:color_picker("Player color", 0, 255, 0)
local teammate_color = menu:color_picker("Teammate color", 0, 0, 255)
local enemy_color = menu:color_picker("Enemy color", 255, 0, 0)

local teammate_fx_value = false
local teammate_fx = menu:checkbox("Teammate FX")

teammate_fx:add_callback(function()
	teammate_fx_value = teammate_fx()
end)

local enemy_fx_value = false
local enemy_fx = menu:checkbox("Enemy FX")

enemy_fx:add_callback(function()
	enemy_fx_value = enemy_fx()
end)

local enable_tracer = menu:checkbox("Enable tracer")

local segmentation_value = 0
local segmentation = menu:slider("Tracer segmentation", 1, 10, {
default = 2,
scale = 0.01
})

segmentation:add_callback(function()
	segmentation_value = segmentation() * 0.01
end)

local splash_count_value = 0
local splash_count = menu:slider("Impact splash count", 10, 60, {
	default = 15
})

splash_count:add_callback(function()
	splash_count_value = splash_count()
end)

local sticky_splash_value = false
local sticky_splash = menu:checkbox("Sticky impact splash")

sticky_splash:add_callback(function()
	sticky_splash_value = sticky_splash()
end)
...
--region main
local illusion = require("havoc_illusion")("tracer fx")

client.set_event_callback("bullet_impact", function(data)
	if (enable_script() == false or illusion:available() == false) then
		return
	end

	local shooter = client.userid_to_entindex(data.userid)
	local shooter_vector = illusion.vector(entity.get_prop(shooter, "m_vecOrigin"))
	local fire_vector = illusion.vector(data.x, data.y, data.z)
	local duck_amount = entity.get_prop(shooter, "m_flDuckAmount")

	shooter_vector.z = shooter_vector.z + (46 + (1 - duck_amount) * 18)

	local shader_value = {0, 0, 0}

	if (shooter == entity.get_local_player()) then
		if (player_fx_value == false) then
			return
		end

		shader_value = {player_color()}
	elseif (entity.is_enemy(shooter) == true) then
		if (enemy_fx_value == false) then
			return
		end

		shader_value = {enemy_color()}
	elseif (entity.is_enemy(shooter) == false) then
		if (teammate_fx_value == false) then
			return
		end

		shader_value = {teammate_color()}
	end

	if (enable_tracer() == true) then
		local tracer_length = fire_vector:distance(shooter_vector)
		local segments = shooter_vector:segment_ray(fire_vector, tracer_length * segmentation_value)

		for i, segment_vector in pairs(segments) do
			if (i > 2) then
				local tracer = illusion:create()

				tracer.origin = segment_vector:clone_offset(
					client.random_float(-2, 2),
					client.random_float(-2, 2),
					0
				)

				tracer.shader = illusion.shader.rgb(
					shader_value[1], shader_value[2], shader_value[3], client.random_int(10, 255),
					false
				)

				tracer.type = "circle"
				tracer.radius = 1.33
				tracer.timer = illusion.timer_standard(true)
				tracer.fade_time = 0.5
				tracer.fade_speed = client.random_float(1, 2)

				tracer.target_vector = illusion.vector(
					client.random_float(-0.2, 0.2),
					client.random_float(-0.2, 0.2),
					client.random_float(0, -0.1)
				)

				tracer.on_frame = function()
					tracer:kill_invisible()
					tracer.shader:fade_out(tracer.fade_speed * illusion.simulation.delta)

					tracer.timer:event(0.1, function()
						tracer.target_vector(
							client.random_float(-0.1, 0.1),
							client.random_float(-0.1, 0.1),
							client.random_float(-0.05, -0.15)
						)

						tracer.timer:restart()
					end)

					tracer.origin = tracer.origin + tracer.target_vector * illusion.simulation.delta
				end
			end
		end
	end

	-- Offset to prevent splash being stuck inside walls.
	local offset = illusion.shared.player_camera_angles:to_backward_vector() * 10

	fire_vector = fire_vector + offset

	for _ = 1, splash_count_value do
		local splash = illusion:create()

		splash.origin = fire_vector
		splash.shader = illusion.shader.rgb(
			shader_value[1], shader_value[2], shader_value[3], client.random_int(10, 255),
			false
		)
		...
		if (sticky_splash_value == true) then
			splash.stuck = false
